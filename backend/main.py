...
async def get_feature_from_image_text(request: Request):
    content = await request.json()
    img = content["img"]
    query = content["query"]
    search_type = content["type"]
    show_result = int(content["number"])
    return search_functions.get_results_search_by_image_and_text(
        img, query, search_type, show_result
    )


@app.get("/index_count")
async def index_count():
    return search_functions.get_index_count()
